...
export class MqttService {
  ...
  async handleMessage(@Payload() data: any, @Ctx() context: MqttContext) {
    ...
    try {
      ...
      const settings = await this.settingService.getThresholds(
        1,
        parsedData.email,
      );
      ...
      if (parsedData.rainSensor && userSettings.autoCloseOnRain) {
        ...
        // Gửi thông báo Pushsafer khi phát hiện mưa
        await this.pushsaferService.sendRainAlert();
        console.log('📲 Đã gửi Pushsafer notification: Cảnh báo mưa');
      }

      if (parsedData.temperature > userSettings.autoCloseTemperature) {
        alerts.push(
          `🌡️ Nhiệt độ cao (${parsedData.temperature}°C > ${userSettings.autoCloseTemperature}°C)`,
        );
      }

      if (parsedData.humidity > userSettings.autoCloseHumidity) {
        alerts.push(
          `💧 Độ ẩm cao (${parsedData.humidity}% > ${userSettings.autoCloseHumidity}%)`,
        );
      }

      if (parsedData.light < userSettings.minLightLevel) {
        alerts.push(
          `🌙 Ánh sáng thấp (${parsedData.light} < ${userSettings.minLightLevel} lux)`,
        );
      }
